...
class Graph:
    ...
    def remove_edge(self, edge):
        ...
        if r_edge in self.edges:
            self.edges.remove(r_edge)
    ...
    def smooth(self):
        ...
        for edge in self.edges:
            ...
            p0, p1 = np.array(self.points[n0]), np.array(self.points[n1])
            
            nr_edges = 3
            new_points = [p0 * (1 - t/nr_edges) + p1 * (t/nr_edges) for t in range(1, nr_edges)]
            
            
            new_nodes = []
            for new_point in new_points:
                new_graph.add_point(new_point)
                new_nodes.append(len(new_graph.points) - 1)
            
            for i,j in zip([n0] + new_nodes, new_nodes + [n1]):
                new_graph.add_edge(i,j)
            
        for n,_ in enumerate(self.points):
            print(f"===={n = }")
            new_bourders = new_graph.neighbors(n)
            print(f"{new_bourders = }")
            points = [new_graph.points[n] for n in new_bourders] + [ new_graph.points[n] ]
            weights = [1] * len(new_bourders) + [2]
            new_graph.points[n] = np.average(points,weights=weights, axis=0)
        
        for n,_ in enumerate(self.points):
            delta = np.array(self.points[n]) - np.array(new_graph.points[n])
            for i in new_graph.neighbors(n):
                new_graph.points[i] = np.array(new_graph.points[i]) + delta
            new_graph.points[n] = self.points[n]
...
with open('test_OGM_04.svg', 'w') as f:
